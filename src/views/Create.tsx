...
export default function Create() {
  const [selectedSize, setSelectedSize] = useState<'banner' | 'square' | 'vertical'>('banner');
  ...
  function handleSubmit() {
    ...
    img.onload = function() {
      let valid = false;
      if (selectedSize === 'banner' && img.width === 1456 && img.height === 180) valid = true;
      if (selectedSize === 'square' && img.width === 1456 && img.height === 1456) valid = true;
      if (selectedSize === 'vertical' && img.width === 180 && img.height === 1456) valid = true;
      if (levelValid !== true) {
        alert('Please enter a valid Level ID.');
        return;
      }
      if (!valid) {
        alert(`Image dimensions do not match the selected size! Expected ${selectedSize} dimensions are: ${getExpectedDimensions(selectedSize)}`);
        return;
      }
  ...
  return (
    <>
      ...
      <p className="text-lg mb-6">Select the size and upload an image for your advertisement.</p>
      <p className="text-lg mb-6">You can only upload one advertisement each size and it expires after 7 days.</p>
      <div className="form-group mb-6">
        <label className="text-lg font-bold mb-2 block">Advertisement Size</label>
        <select 
          className="custom-select"
          value={selectedSize}
          onChange={(e) => setSelectedSize(e.target.value as 'banner' | 'square' | 'vertical')}
        >
          <option value="banner">Banner (1456 x 180)</option>
          <option value="square">Square (1456 x 1456)</option> {/* ill figure out the ratio  for this later */}
      ...
      <div className="form-group mb-6">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '16px' }}>
          ...
          <div style={{ marginLeft: '8px', alignSelf: 'flex-start', fontSize: '0.95em' }}>
            {imageName && (
              <span>
          ...
          <div className="image-preview" style={{ display: 'flex', justifyContent: 'center', width: '100%' }}>
            <img src={imagePreview || './assets/blacksquare.png'} alt="No Image Selected" />
          </div>
        </div>
      </div>
      <div className="form-group mb-6">
      ...
      <div className="form-group mb-6" style={{ display: 'flex', justifyContent: 'center' }}>
